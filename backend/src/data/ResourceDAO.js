...
function getCourseFiles(scr_sec_number, scr_crs_id) {
    ...
    return db.query('SELECT fil_id, fil_link FROM section_resource JOIN file ON scr_fil_id = fil_id WHERE scr_sec_number = ?', [scr_sec_number, scr_crs_id]).then(({results}) => {
        console.log(results);
